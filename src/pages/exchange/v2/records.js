              ...
              rows.map((item,index)=>(
                <div className={styles.records_detail} key={index}>
                  <div className={styles.ex_header}>
                    ...
                    <span className={classnames({[styles.add]: item.dtype === 2 || item.dtype === 4 || item.dtype === 6 || item.dtype === 7})}>
                      {item.dtype === 2 || item.dtype === 4 || item.dtype === 6 || item.dtype === 7 ? '+' : '-'}{item.score}
                    </span>
                  </div>
                  {
                    item.orderBill && item.orderBill.detailList.map((good,index)=>(
                      <div className={styles.goods_detail_goods} key={index}>
                        ...
                        <div className={styles.goods_desc}>
                          <p>{{"1": "获得积分", "2": "积分兑换", "3": "获赠", "4": "赠出", "5": "冲正(+)", "6": "冲正(-)", "7": "退货", "8": "退货", "9": "订单退单", "10": "作废订单", "11": "取消订单" }[item.dtype]}</p>
